...
import com.lipiji.mllib.layers.Type;
	...
	public void bptt(Map<String, DoubleMatrix> acts, int lastT, double lr) {
		for (int t = lastT; t > -1; t--) {
			...
			if (t == lastT) {
				...
				deltaH = Why.mmul(deltaY.transpose()).transpose().add(Whc.mmul(lateDgc.transpose()).transpose()).add(Whi.mmul(lateDi.transpose()).transpose()).add(Who.mmul(lateDo.transpose()).transpose()).add(Whf.mmul(lateDf.transpose()).transpose());
			}
			...
			if (t == lastT) {
				...
				DoubleMatrix lateDi = acts.get("di" + (t + 1));
				deltaC = deltaH.mul(o).mul(deriveTanh(gh)).add(Wco.mmul(deltaO.transpose()).transpose()).add(lateF.mul(lateDc)).add(Wcf.mmul(lateDf.transpose()).transpose()).add(Wci.mmul(lateDi.transpose()).transpose());
			}
