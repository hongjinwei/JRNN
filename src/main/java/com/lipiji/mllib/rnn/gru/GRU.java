	...
	public void bptt(Map<String, DoubleMatrix> acts, int lastT, double lr) {
		for (int t = lastT; t > -1; t--) {
			...
			if (t == lastT) {
				...
				deltaH = Why.mmul(deltaY.transpose()).transpose().add(Whr.mmul(lateDr.transpose()).transpose()).add(Whz.mmul(lateDz.transpose()).transpose()).add(Whh.mmul(lateDgh.mul(lateR).transpose()).transpose()).add(lateDh.mul(z));
			}
